...
> 当前状态：**P0 早期开发阶段。** 本地应用、TXT/来源链路、结构化分镜、角色/风格审批、NovelAI 固定契约、有界串行队列、逐格图像执行器、不可变素材版本和本地确定性页面合成已实现并通过离线 Mock 验收。真实付费图像调用尚未经用户单独批准或验收；reroll、inpaint、历史激活和导出仍未实现。
...
| reroll、inpaint、历史激活、导出 | 未实现 | 不得把技术文档中的设计当作可用功能 |
...
启动器会选择本机端口并打开浏览器。运行数据默认写入 `~/Library/Application Support/Manga Maker/`，不会写入仓库。当前界面可以管理本地加密凭证、导入与改编章节、审批角色/风格、配置 NovelAI，预检、启动、二次确认执行有界队列，并把已有面板素材排成可编辑漫画页。连接测试只查标签；只有点击“执行冻结队列”并完成明确确认才会进入图像请求路径。页面文字、格框和裁切的保存只在本机创建新 PageVersion，不产生真实费用。
