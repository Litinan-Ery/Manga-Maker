...
> 当前状态：**P0 早期开发阶段。** 本地应用骨架、加密凭证库、项目/TXT 导入、来源追溯、结构化分镜、角色/风格审批、NovelAI 无出图连接测试，以及有界串行队列状态机已实现并通过 Mock 验收。付费图像生成执行器、排版、reroll 和导出尚未实现；尚未执行过真实文本模型或 NovelAI 调用。
...
| 有界串行生成队列 | 已实现（本地状态机） | 冻结面板和所有上游版本、调用/成本上限及用户动作；全局单在途、暂停/取消、重启转人工审阅；尚未接图像执行器 |
| NovelAI 付费出图、排版、reroll、导出 | 未实现 | 不得把技术文档中的设计当作可用功能 |
...
启动器会选择本机端口并打开浏览器。运行数据默认写入 `~/Library/Application Support/Manga Maker/`，不会写入仓库。当前界面可以管理本地加密凭证、创建项目、导入 TXT、修正章节、建立来源覆盖账本、生成和审批结构化分镜，编辑和独立审批角色设定与黑白风格板，保存 NovelAI 模型配置，并预检与控制有界队列。NovelAI 连接测试只查标签；当前队列没有图像执行器，二者均不会出图。
