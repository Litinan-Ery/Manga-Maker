...
> 当前状态：**P0 的离线 Mock 单章闭环与 P1 整本规划已完成，真实服务验收仍未执行。** 本地应用、TXT/来源链路、结构化分镜、角色/风格审批、有界生成、页面编辑、reroll/inpaint、版本恢复、四格式导出、跨章连续性、整本预算/逐章队列、启动恢复、磁盘故障处理和凭证零泄露扫描均已有自动化证据。真实 NovelAI 付费 smoke 与代表性授权章节的真实生产仍需用户单独批准，不能由 Mock 结果替代。
