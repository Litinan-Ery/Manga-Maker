...
本轮按 UC-01 → UC-02 → UC-03 → UC-04 串行执行，前项的定向用例通过后才开始下一项。

| 用例 | 修复 | 当前证据 |
|---|---|---|
| UC-01 | 批准版式、格框、焦点与文字区用于正式拼页 | 定向用例通过；新旧版本及零图像调用回归通过 |
| UC-02 | 自然语言描述与镜头语义进入实际 Provider payload | 固定 seed 下四类视觉变化可见，旧哈希兼容，Inspector 与执行一致 |
| UC-03 | V5 Full 整页多格入口、文字策略、预览冻结、一次生成和审阅采用 | 两种文字策略、幂等、未知结果、来源失效、四格式导出与恢复后编辑通过 |
| UC-04 | 原创两页三格/四格漫画用例、冲突 UC 预设与跨模式串行修复 | 7 次真实图像调用；最终两页采用无字底图与本地日语排字，四格式导出及隔离恢复通过 |
| UC-04A | 整页行列与方位表达、角色具体服装、移除默认构图重复项 | 同 seed 三格由四条横格恢复为上排两格/下排通栏；四格拓扑通过，非像素级几何保证 |
| UC-04B | 逐格文字与全页方位对应表，模型文字边界 | 精确模型文字仍有串格；入口标为实验，推荐本地文字，最终对白归属通过本地排字实现 |
| UC-04C | 本地气泡不打印角色名前缀，每句对白保留独立说话人元数据 | 旧代码失败用例已转通过；全后端 346 项、前端 49 项、ruff/mypy/build 通过 |

验收数字、命令与可打开产物见 [分镜修复验收记录](docs/storyboard-repair-cases.md)。
真实成品与逐次失败/成功证据见 [《雨夜的钥匙》验收报告](.manga-maker/storyboard-real-final/README.md)。
这批代码修复不等于候选质检/PageApproval 或整体 v0.3 已交付。
...
|---|---|---|---|
| 长任务上下文与恢复 | FR-24、NFR-07、AC-13、PRD §25、架构 §23 | MM-072～MM-078 | 本地预算、检查点、失败交接、恢复核对、接口/界面及故障注入通过；真实宿主和真实 100 页验收分别记录 |
| 必备封面 | FR-25、AC-14、PRD §26、架构 §25 | MM-081～MM-084 | 独立封面必备、预算不挤占正文、四格式首位与正确计数、可编辑恢复及成品验收；新增待实现 |
| 启动前画家风格确认 | FR-26、AC-15、PRD §26、架构 §25 | MM-079～MM-080、MM-084 | 先询问再启动、同一确认版本传到封面/正文、所有入口与恢复可验证、变更使相关批准失效；新增待实现 |
...
<a id="context-recovery-tickets"></a>

## FR-24 / 长任务上下文与恢复（2026-09-06）

执行顺序：MM-072 → MM-073 → MM-074 → MM-075 → MM-076 → MM-077；MM-078 为独立真实集成验收。保留原有未提交变更及真实生产文件，开发测试使用隔离工作区和模拟供应商。

| Ticket | 状态 | 交付与验证 |
|---|---|---|
| MM-072 | Done | 预算策略、图片限制、聚合结果 envelope；10 项定向测试及 Ruff 通过；整体回归见 MM-077 |
| MM-073 | Done | 检查点、迁移、所有权、并发与发布中租约失效测试通过；最终静态/整体回归见 MM-077 |
...
- 依赖：FR-24 / 架构 §23.3。
- 交付：版本化策略、实际/估算/滞后观测、请求增量去重、输出预留、50%/60%/70% 水位、图片计数及有界聚合结果。
- 验收：单次与聚合超限、窗口未知/过小、压缩后重新计数、截断区域含失败项；完整证据可定位，错误总数准确。

### MM-073 检查点持久化与单写者

- 依赖：MM-072。
- 交付：workflow 所有的检查点/执行状态、不可变文件与哈希、CAS revision、租约及 fencing；源与结果仅保留引用。
- 验收：并发领取、旧执行者晚提交、半写入、缺失/损坏 checkpoint、重启后读取及 stale revision；失败不覆盖有效记录。

### MM-074 压缩失败状态机与宿主能力

- 依赖：MM-073。
- 交付：可替换 ContextHostPort、本地不支持适配器、可注入测试宿主、持久重试次数与退避、压缩后观测验证、最小恢复包。
- 验收：连续两次失败即交接、不可重试错误直接交接、重启不清零、缺少宿主能力时手动恢复入口可用，模型不可用仍可读检查点。

### MM-075 应用接口与恢复核对

- 依赖：MM-073/074。
- 交付：typed container 接线、摘要与证据分页、检查点及恢复计划/恢复包/继续接口；从领域公开查询核对生产、页面、来源和在途状态。
- 验收：跨项目/非法引用/缺少 session/CSRF 被拒绝；ready、文字和视觉状态分离；未知生成不触发 POST；过期计划或输入变更阻止恢复。

### MM-076 生产脚本与前端入口

- 依赖：MM-075。
- 交付：脚本单写者接入、每页检查点及有界批次；本地摘要/交接 CLI；前端生成/排版/审查进度、检查点和可操作恢复状态。
- 验收：现有脚本行为回归、第二写者拒绝、重启不重复生成；UI 正确展示未验收和无宿主能力状态，不请求未知接口或隐式生成。

### MM-077 故障注入与回归验收

- 依赖：MM-072～076。
- 交付：UC-CTX-01～10 可重复测试、隔离 100 页模拟流水线、定向/全量后端与前端测试、静态检查、构建和验收报告。
- 验收：记录通过与未覆盖边界，完整目标/限制和页序/哈希在恢复后保持；真实供应商请求数单独报告。
- 完成证据：[长任务恢复验收报告](docs/context-recovery-acceptance.md)。100 页脚本/API 模拟丢失第 26 页响应后继续，100 个生成请求各只提交一次；45 项恢复相关后端测试覆盖边界与故障注入，新增真实供应商请求为 0。

### MM-078 真实宿主与真实整书集成

- 依赖：MM-077，以及实际宿主控制能力、现有真实生产任务的执行权和有效授权。
- 交付：真实压缩/替换的触发与恢复证据、100 页真实逐页/连续性/导出验收。
- 验收：缺少宿主接口时报告手动交接已交付、自动替换未验收；真实成品按 PRD 第 24 节验收，不以此次开发测试替代或重复启动已有真实生产。

<a id="cover-and-artist-style-tickets"></a>

## FR-25/26：封面与启动前画家风格（需求拆分，2026-09-06）

本轮新增 PRD 与架构定义，下列工单均为 `Todo`，尚未编码或进行功能验收。优先级为 P0；沿用公开 Port、审批冻结、单在途、不可变版本和检查点的边界，不以已有 `cover` 页型或 StyleBible 代替新功能。

| Ticket | 状态 | 依赖 | 交付 |
|---|---|---|---|
| MM-079 | Todo | FR-26 | 项目级 ArtistStyleIntent、启动前询问/确认、有效版本恢复、API/CLI 公共启动门禁 |
| MM-080 | Todo | MM-079 | 风格意图传播至分镜、设定、封面/正文 Prompt 与执行审批；变更、并发与过期引用处理 |
| MM-081 | Todo | MM-079/080 | CoverBrief、封面布局与 BookComposition 契约、正文/封面独立身份、预算和进度 |
| MM-082 | Todo | MM-081、现有 production/composition/review 公开契约 | 封面候选生产、中文标题本地排版、版本与审查、独立修改/重绘 |
| MM-083 | Todo | MM-080/081/082、MM-073/075 | 带封面四格式导出、工程包/检查点兼容和后继任务恢复，完整正文保真 |
| MM-084 | Todo | MM-079～083 | 启动/失效/导出/恢复端到端回归、UI 实测和授权真实封面/风格一致性验收 |

### MM-079 启动前画家风格意图与确认

- 交付：由 world_bible 保存独立于 Storyboard 的意图版本；生产入口提问、草稿/确认/取消、明确不指定、历史项目补问；确认不自动发模型请求。
- 验收：UC-STYLE-01/02/03，未确认时 UI/API/CLI/恢复均阻止生产，有效确认重启后保留；读取/导入和历史记录保持可用。

### MM-080 风格意图传播与失效

- 交付：StyleBible/PromptPlan/GenerationApproval 的精确意图引用、确定性传播、lineage 失效和发送前版本核验；保留旧版本及在途结果来源。
- 验收：UC-STYLE-02/04，封面与正文不擅自换画家；旧确认/批准不能发新请求，修改与执行竞争不混入新风格成品，既有历史不删除。

### MM-081 独立封面计划、书目与预算

- 交付：CoverBrief、特殊页版式和 BookComposition 的公开契约及迁移；独立封面身份、唯一封面引用、正文/封面计数、启动预检和单独进度。
- 验收：UC-COVER-01/02，100 正文 + 1 封面独立确认，共 101 张；不使用 page 0、虚假章节或正文第 1 页占位，不缩减正文预算。

### MM-082 封面生成、排版与审查

- 交付：封面接入已有串行 production、成本/审批与未知结果处理；可编辑书名图层、不可变封面 PageVersion 和有效审查引用。
- 验收：UC-COVER-03/04，封面有真实内容、标题准确可读、与正文风格一致；标题修改不出图，重绘只影响封面，ready 不等于批准。

### MM-083 封面导出与任务/工程恢复

- 交付：四格式共享书目冻结顺序，工程包保存封面编辑结构与风格意图；新旧 schema 读取、封面单元/风格引用与显式后继运行。
- 验收：UC-COVER-02/03/04、UC-STYLE-03；首张封面唯一、正文页序/哈希不变；缺封面阻断正式成品但可备份；旧 100 页检查点升级后不丢正文或审查证据。

### MM-084 新需求端到端与实际成品验收

- 交付：UC-COVER-01～04、UC-STYLE-01～04 的契约、门禁、并发/重启、离线 E2E、前端与视觉/导出验收报告。
- 验收：明确区分新旧版本、模拟与真实证据；至少覆盖封面 + 100 页正文的导出/恢复、封面与代表性正文的画风核对。真实图像请求须沿用已有明确授权和预算，不因该工单自动获得授权；缺真实验收时工单保持未完成。
...
## 2026-09-06：《莱博维茨的赞歌》100页实物验收

PRD第24节UC-100-01～04及续产新增03A～03E、04A已完成本次真实100页生产验收。100页/300个中文图层、四格式导出、秘密扫描、独立工程包恢复与修改旁白均通过，证据见[100页验收报告](docs/canticle-100-acceptance.md)。保留50张未采用候选和一次出图前订阅失败记录；没有重置未知状态或自动重发图像请求。本条完成MM-078所涉及的真实整书产物部分，不改变该工单对真实宿主压缩/替换证据的独立要求。
