...
| 文档版本 | v0.1 |
| 日期 | 2026-08-09 |
| 产品状态 | P0 离线 Mock 单章闭环已完成；P1 跨章节连续性账本、整本预算与逐章恢复已实现；真实 NovelAI 付费调用与代表性授权章节的真实生产仍未验收 |
...
当前实现将完整章节页序和每个 `PageVersion` 的渲染哈希写入 `ExportRevision`。逐页 PNG 保留规范 2048 × 3072 内容，PDF 使用同一批规范 PNG 合成，CBZ 包含零填充 PNG 与 `ComicInfo.xml`；四种结果全部通过打开、页数、页序和哈希检查后才发布。
...
整本计划不是无人值守批处理授权。创建前必须冻结当前章节集、贯穿末章的已批准连续性版本、每章分镜/角色/风格版本、预计页数/分格数和全书调用/成本硬上限；每章再独立批准。启动后一次人工推进最多创建一个已有单章有界任务，图像执行仍需生成控制台再次确认。重启只把活动计划暂停或把未知计费章节转 `needs_review`，不得自动创建下一章、重新发送未知请求或突破全书上限。
