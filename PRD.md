...
| 文档版本 | v0.1 |
| 日期 | 2026-08-09 |
| 产品状态 | P0 早期开发阶段；导入、来源追溯、结构化分镜、角色/风格审批、NovelAI 无出图连接测试及有界串行队列状态机已通过 Mock 验收，完整漫画闭环尚未实现 |
...
- MM-011 已实现的边界：固定 `https://image.novelai.net/docs/doc.json` 的审计哈希和映射版本；项目只保存模型 ID、vault profile 引用和非敏感连接状态。连接测试必须由用户点击，只调用标签建议接口，生成图片为 0，且不自动重试。付费出图仍未实现。
...
- MM-012 已实现全局单在途领取、乐观 revision、暂停/恢复/取消、上限门禁和启动 reconciliation；应用重启不会自动领取 queued 项，运行但无在途的队列转为 paused，在途项转为 needs_review。实际 NovelAI 调用、错误重试和素材落盘属于 MM-013。
...
P0 采用 Python/FastAPI 本地后端、SQLite 元数据、React/TypeScript 前端和本地不可变文件存储。技术栈是未来实现基线，不代表已经创建。
