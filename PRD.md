...
- 已实现边界：固定 `https://image.novelai.net/docs/doc.json` 的审计哈希和映射版本；项目只保存模型 ID、vault profile 引用和非敏感连接状态。连接测试只调用标签建议与订阅接口，不出图；出图路径仅在有界 Job 进入 `running` 后、用户再次明确确认时调度。当前仅完成离线 Mock 验收，尚未执行真实付费调用。
